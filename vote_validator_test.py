...
def test_rule_1(validator):
    ...
    """
    votes = validator.votes[validator.id]  # dict {validator_id: votes}

    # Dict mapping height to votes targeting this height
    height_to_vote = {}
    for vote in votes:
        assert vote.validator == validator.id,\
            "validators don't match: %d and %d" % (
                vote.validator, validator.id)

        if vote.target_height not in height_to_vote:
            height_to_vote[vote.target_height] = []

        height_to_vote[vote.target_height].append(vote)
...
def test_rule_2(validator):
    ...
    for height in heights:
        ...
        c1 = vote.source_height
        for c3 in heights:
            if c1 < c3 and c3 < c4:
                vote_bis = height_to_vote[c3]
                c2 = vote_bis.source_height
                assert c2 <= c1, "Rule 2 is broken ! %d, %d, %d, %d" % (
                    c1, c2, c3, c4)


def is_finalised(validator, hash_):
    """Returns true if the `hash_` corresponds to a finalised block for validator.

    A checkpoint c in finalised if it is justified and there exist a supermajority link
